...
class Actions():
    ...
    def beameffect(self, bullet, target, shooters, addangle=None):
        ...
        for shooter in shooters:
            ...
            dest_x = self.player_list[target].center_x
            dest_y = self.player_list[target].center_y
            print(f"{self.player_list[shooter].name}:{start_x}:{start_y} to {self.player_list[target].name}:{dest_x}:{dest_y}")
            ...
            bullet.angle = math.degrees(bullet_angle) - 90
            if addangle:
                bullet.angle += addangle
            print(f"Bullet angle: {bullet.angle:.2f}")

            # Taking into account the angle, calculate our change_x
            # and change_y. Velocity is how fast the bullet travels
            # print(f"Bullet angle: {angle:.2f}")
            bullet.change_x = math.cos(angle) * BULLET_SPEED
            bullet.change_y = math.sin(angle) * BULLET_SPEED
